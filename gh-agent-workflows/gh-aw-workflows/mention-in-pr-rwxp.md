...
- **Important**: Substantive feedback belongs in the PR review (inline comments + review submission), NOT in a reply comment. Do NOT add a separate comment after submitting the review unless the user explicitly asked for a comment or the review submission failed. If you do comment, keep it to a brief status (e.g., "review submitted: requested changes").
...
If you did not submit a PR review, call `add_comment` with your response. If you submitted a review, only call `add_comment` when explicitly requested or to report a review submission failure.
