  ...
  ready-to-push-to-pr:
    ...
    py: |
      ...
      if diff_line_count > 0:
          checklist.append(f'A diff of your unpushed changes ({diff_line_count} lines) has been saved to `/tmp/self-review/diff.patch` (full diff) and `/tmp/self-review/stat.txt` (summary). Spawn a `code-review` sub-agent via `runSubagent` to review the diff against the codebase. Tell it to read `/tmp/self-review/diff.patch` and the relevant source files, and look for bugs, logic errors, missed edge cases, and style issues. IMPORTANT: In your sub-agent prompt, include your own context so the reviewer starts warm instead of cold: what you changed and why, which files are relevant and what they do, edge cases you already considered and why they are handled, and what the tests cover. This helps the reviewer focus on what you might have MISSED rather than re-deriving your full reasoning from scratch. If the sub-agent finds legitimate issues, fix them, commit, and call `ready_to_push_to_pr` again to regenerate the diff before proceeding.')
