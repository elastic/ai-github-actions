...
1. **Build a candidate set**
   ...
   Start with high-signal open issues instead of scanning everything:
   ```
   github-search_issues: query="repo:{owner}/{repo} is:issue is:open linked:pr"
   github-search_issues: query="repo:{owner}/{repo} is:issue is:open in:comments (fixed OR resolved OR closed)"
   github-search_issues: query="repo:{owner}/{repo} is:issue is:open updated:<YYYY-MM-DD"
   ```

   Paginate each query (use `per_page: 20`), collect issue numbers, and de-duplicate.
   Use today's date minus 90 days (YYYY-MM-DD) for the `updated:<` filter.
