...
imports:
  ...
  - gh-aw-fragments/workflow-edit-allowed.md
...
safe-outputs:
  ...
  # Full create-pull-request block (mirrors safe-output-create-pr.md) so exclude
  # does not replace draft / patch-format / extra-commit token wiring.
  create-pull-request:
    patch-format: bundle
...
2. If you implemented a valid fix with verification (including workflow / `.github/` changes), call `ready_to_make_pr` and then `create_pull_request` to open a draft PR. Do not finish with comment-only when a verified fix is ready to land as a PR.
