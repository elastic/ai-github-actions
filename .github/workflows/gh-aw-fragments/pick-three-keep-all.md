...
**How to spawn sub-agents:** Call `runSubagent` with `model: "${{ inputs.model }}"` and `agentType: "general-purpose"` (unless the workflow specifies a different agent type). Always pass the model explicitly. Sub-agents cannot see your conversation history, the other sub-agents' results, or any context you have gathered so far. Each prompt must be **fully self-contained** — include everything the sub-agent needs:
