...
- **Fork PRs**: Cannot push to fork PR branches. Check via `pull_request_read` with method `get` whether the PR head repo differs from the base repo. If it's a fork, use `create_pull_request` to open a new PR in the base repo with the changes instead. Reference the original fork PR in the description.
